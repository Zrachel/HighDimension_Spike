...
%   where result_file_names = {'file_name1', 'file_name2', ...}.  
%
...
%      errorbar file if errorbars are to be plotted).  
%
  ...
  properties 
    ...
    methods 
        ...
        function plot_results(plot_obj)
            ...
            if length(plot_obj.result_file_names) > length(plot_obj.the_colors)
            ...
            for iResult = 1:length(plot_obj.result_file_names)
               ...
               %  use these parameters to create a time interval
               if isstruct(curr_time_interval) || ~isempty(curr_default_binning_parameters)
            ...
            for iErrorBar = 1:length(plot_obj.errorbar_file_names)
                
                % more sanity checks
                if length(plot_obj.errorbar_file_names) ~= length(plot_obj.result_file_names)
            ...
            if ~isempty(plot_obj.p_values)                
                ...
                if length(plot_obj.result_file_names) ~= length(plot_obj.p_values)
                    error('plot_obj.p_values must be a cell array that is the same length as plot_obj.result_file_names')
                end
